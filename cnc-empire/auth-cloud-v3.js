...
let supabase=null,session=null,gameLoaded=false,cloudTimer=null,cloudBusy=false,lastUploaded='',onlineSystems=null;
...
function hardBalance(code){
  const replacements=[
    ...
    ["${tab==='shop'?shop():tab==='orders'?orders():tab==='board'?board():profile()}","${tab==='shop'?shop():tab==='orders'?orders():tab==='board'?board():tab==='online'?(window.CNC_ONLINE?.render?.()||'<div class=\"notice\">Online-Systeme laden …</div>'):profile()}"],
    ["const n=[['shop','⚙️','Werkstatt'],['orders','📋','Aufträge'],['board','🏆','Rangliste'],['profile','👤','Profil']];","const n=[['shop','⚙️','Werkstatt'],['orders','📋','Aufträge'],['board','🏆','Rangliste'],['online','🌐','Online'],['profile','👤','Profil']];"],
    ["if(x==='board'||x==='profile')loadFriends()","if(x==='board'||x==='profile')loadFriends();if(x==='online')window.CNC_ONLINE?.refresh?.().then(()=>render())"]
  ];
...
async function enterGame(newSession){if(!newSession?.user)return;session=newSession;hideAuth();try{await reconcileSave();await initOnlineSystems();await loadGame()}catch(err){console.error('CNC season2 load',err);gameLoaded=false;setCloud('Cloud-Fehler','error');$('root').innerHTML='<div class="notice"><b>Saison 2 konnte nicht geladen werden.</b><br>Bitte Verbindung prüfen und Seite neu laden.</div>'}}
