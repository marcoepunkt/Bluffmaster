...
function normalize(raw){
  ...
  return {...d,...x,staff,inventory:inv,material,shift,lastTick:Number(x.lastTick)||Date.now(),totalShipped:Math.max(0,Number(x.totalShipped)||0),scrap:Math.max(0,Number(x.scrap)||0),chainRevenue:Math.max(0,Number(x.chainRevenue)||0)};
}
...
function productionMultiplier(){
  ...
  return Math.min(2.5,staffMult*shift.mult);
}
...
function render(){
  ...
  return `<div id="operationsSystems"><div class="notice"><b>👥 Betriebsführung aktiv.</b> Mitarbeiter und Schichten verstärken deine vorhandene Fertigung. Produktionsfaktoren wirken direkt auf Maschinen und Aufträge.</div><div class="grid"><div class="card stat"><div class="label">Mitarbeiter</div><div class="value">${staff}</div><div class="sub">5 Fachbereiche</div></div><div class="card stat"><div class="label">Produktionsfaktor</div><div class="value">×${mult.toFixed(2)}</div><div class="sub">${esc(SHIFTS[S.shift]?.name||'Frühschicht')}</div></div></div><div class="section">Schichtmodell</div><div class="opsShifts">${Object.keys(SHIFTS).map(shiftCard).join('')}</div><div class="section">Mitarbeiter</div><div class="list">${Object.keys(ROLES).map(staffCard).join('')}</div>${renderChain()}</div>`;
}
function start(){
...
export async function initCncOperations(ctx){
  ...
  const api={render,refresh,hire,setShift,setMaterial,buyMaterial,productionMultiplier,state:S,start,stop};
