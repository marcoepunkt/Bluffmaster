...
assert(community.includes('markPollsRead')&&community.includes('openPolls'),'opening Polls marks open polls as read');
...
assert(community.includes("upsert({poll_id:pollId,user_id:user.id")||community.includes("upsert({poll_id:pollId,user_id:user.id,"),'poll creator is marked as having read own poll');
...
assert(read('v3part5.txt').includes('renderNewsAlert'),'Workshop renders Community unread alert');
