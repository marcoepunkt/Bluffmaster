...
function optimisticRoomPatch(patch) {
  roomState = { ...roomState, ...patch };
  renderGame();
}

async function updateRoomFast(patch, statusText = "Aktion wird übertragen …") {
  if (actionInProgress) return;

  actionInProgress = true;
  const previousRoomState = { ...roomState };
  optimisticRoomPatch(patch);
  setMessage(gameMessage, statusText);
  ...
  try {
    ...
    renderGame();
    setMessage(gameMessage, "Die Aktion konnte nicht übertragen werden. Bitte erneut versuchen.", "error");
  } finally {
...
async function rollDice() {
  if (
    actionInProgress ||
    roomState.currentPlayerUid !== auth.currentUser.uid ||
    roomState.phase !== "waiting_roll"
  ) return;
  ...
  const previousRoomState = { ...roomState };
  const previousPrivateRoll = privateRoll;

  // Sofortige Anzeige auf dem eigenen Handy, ohne Firebase abzuwarten.
  privateRoll = {
  ...
  setMessage(gameMessage, "Wurf wird übertragen …");

  try {
    // Beide Änderungen werden in nur einem Netzwerk-Schritt übertragen.
    const batch = writeBatch(db);
    batch.set(doc(db, "rooms", currentRoomCode, "privateRolls", auth.currentUser.uid), {
      ...privateRoll,
      createdAt: serverTimestamp()
    });
    batch.update(doc(db, "rooms", currentRoomCode), {
    ...
    console.error(error);
    roomState = previousRoomState;
    privateRoll = previousPrivateRoll;
    renderGame();
    setMessage(gameMessage, "Der Wurf konnte nicht übertragen werden.", "error");
  } finally {
    actionInProgress = false;
...
async function maybeRevealRequestedRoll() {
  ...
  // Der private Wurf ist normalerweise bereits im Speicher.
  // Dadurch entfällt beim Aufdecken ein zusätzlicher Firestore-Lesevorgang.
  let roll = privateRoll;

  if (!roll || roll.ownerUid !== auth.currentUser.uid) {
    const privateSnap = await getDoc(
      doc(db, "rooms", currentRoomCode, "privateRolls", auth.currentUser.uid)
    );

    if (!privateSnap.exists()) return;
    roll = privateSnap.data();
  }
