...
async function initSupabase(){const [{createClient},cfg]=await Promise.all([import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2.105.0/+esm'),readConfig()]);supabase=createClient(cfg.url,cfg.key,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});return supabase}
...
async function loadGame(){if(gameLoaded)return;gameLoaded=true;const root=$('root');root.innerHTML='<div class="notice">Saison 2 wird gestartet …</div>';const files=[1,2,3,4,5,6,7].map(n=>'./v3part'+n+'.txt?v='+GAME_PART_VERSION);const parts=await Promise.all(files.map(f=>fetch(f,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(f+' '+r.status);return r.text()})));new Function(hardBalance(parts.join('')))();const now=readLocalState();if(now)lastUploaded='';installCloudReset();startCloudLoop();await flushCloud(true)}
...
async function enterGame(newSession){if(!newSession?.user)return;session=newSession;hideAuth();try{await reconcileSave();await loadGame()}catch(err){console.error('CNC season2 load',err);gameLoaded=false;setCloud('Cloud-Fehler','error');$('root').innerHTML='<div class="notice"><b>Saison 2 konnte nicht geladen werden.</b><br>Bitte Verbindung prüfen und Seite neu laden.</div>'}}
